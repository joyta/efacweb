jQuery.validator.addMethod("numeroDocumento", function (value, element, param) {
    var validator = this;
    var tipo = $(param+  ' option:selected').val();
    var valid = validarNumeroDocumento(value, tipo);
    
    if (valid !== 'ok') {
        $.validator.messages.numeroDocumento = valid;
...
function validarNumeroDocumento (numero, tipo) {    
    ...
    if(tipo === 'ClienteOcacional' && numero !== '9999999999999'){
        return 'El número del cliente ocacional debe ser: 9999999999999';
    }
    
    if(tipo === 'Pasaporte' && numero.length < 6){
        return 'El pasaporte debe tener como mínimo 6 caracteres';
    }
    ...
    if(tipo === 'Ruc' && !(numero.length >= 13 && numero.length <= 14)){
        return 'El ruc debe tener 13 o 14 dígitos';
    }
    ...
    var codpro = numero.substr(0, 2) * 1;
    if (codpro < 0 || codpro > numeroProvincias){
